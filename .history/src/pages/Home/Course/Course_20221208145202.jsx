import React from "react";

const Course = () => {
  return (
    <div className="bg-white rounded-xl relative shadow-md group/course">
      <a href="overflow-hidden">
        <span className="absolute bg-primary text-white px-2 py-1 rounded">
          72%
        </span>
        <div className="">
          <img
            src="https://static.unica.vn/upload/images/2019/04/khoa-hoc-dem-hat-guitar-can-ban_m_1555570862.jpg"
            alt=""
          />
        </div>
        <div className="p-6">
          <h2 className="font-semibold">Học đệm đàn cùng Kikikta</h2>
          <div className="mt-6 flex items-center justify-between text-[12px] text-lightBlack">
            <span className="">Hà thủ ô</span>
            <span className="line-through">
              700.000<sup>đ</sup>
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span>
              <ul className="flex">
                <li>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#0056d2"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="none"
                    className="w-6 h-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"
                    />
                  </svg>
                </li>
                <li>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#0056d2"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="none"
                    className="w-6 h-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"
                    />
                  </svg>
                </li>
                <li>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#0056d2"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="none"
                    className="w-6 h-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"
                    />
                  </svg>
                </li>
                <li>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#0056d2"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="none"
                    className="w-6 h-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"
                    />
                  </svg>
                </li>
                <li>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
                    fill="#0056d2"
                    viewBox="0 0 24 24"
                    strokeWidth={1.5}
                    stroke="none"
                    className="w-6 h-6"
                  >
                    <path
                      strokeLinecap="round"
                      strokeLinejoin="round"
                      d="M11.48 3.499a.562.562 0 011.04 0l2.125 5.111a.563.563 0 00.475.345l5.518.442c.499.04.701.663.321.988l-4.204 3.602a.563.563 0 00-.182.557l1.285 5.385a.562.562 0 01-.84.61l-4.725-2.885a.563.563 0 00-.586 0L6.982 20.54a.562.562 0 01-.84-.61l1.285-5.386a.562.562 0 00-.182-.557l-4.204-3.602a.563.563 0 01.321-.988l5.518-.442a.563.563 0 00.475-.345L11.48 3.5z"
                    />
                  </svg>
                </li>
              </ul>
            </span>
            <span className="font-semibold">
              200.000<sup>đ</sup>
            </span>
          </div>
        </div>
      </a>
      <div className="w-[340px] absolute top-0 translate-y-[-20%] translate-x-[85%] group-hover/course:visible invisible shadow-md p-10 bg-white z-20">
        <h1 className="font-semibold text-[16px] min-h-[40px]">
          Nhập môn chứng khoán
        </h1>
        <span className="mb-4 px-3 py-2 text-[12px] bg-primary text-white rounded-md">
          Best seller
        </span>
        <p className="line-clamp-3 overflow-hidden">
          Khóa học chứng khoán nhập môn sẽ giúp bạn nắm bắt được kiến thức, đầu
          tư chứng khoán đầy đủ. Bài bản và chi tiết nhất dành cho người mới bắt
          đầu tham gia
        </p>
        <ul>
          <li>
            Kiến thức cơ bản nhất về thị trường chứng khoán, về đầu tư chứng
            khoán và tiềm năng lợi nhuận khổng lồ của TTCK
          </li>
          <li>
            Kiến thức cơ bản nhất về thị trường chứng khoán, về đầu tư chứng
            khoán và tiềm năng lợi nhuận khổng lồ của TTCK
          </li>
          <li>
            Kiến thức cơ bản nhất về thị trường chứng khoán, về đầu tư chứng
            khoán và tiềm năng lợi nhuận khổng lồ của TTCK
          </li>
        </ul>
        <a>Thêm vào giỏ hàng</a>
      </div>
    </div>
  );
};

export default Course;
